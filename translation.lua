...
local Dictionary = {
    ...
    ["Color Customization"] = { PT = "Personalização de Cor", ES = "Personalización de Color" },["Beast highlight Color"] = { PT = "Cor Realce Fera", ES = "Color Resalte Bestia" },
    ...
    -- Visual Page["Camera & UI"] = { PT = "Câmera e Interface", ES = "Cámara y UI" },
    ["Fov Changer"] = { PT = "Mudar FOV", ES = "Cambiar FOV" },
    ...
    ["Hide Leaves (Only Homestead)"] = { PT = "Ocultar Folhas (Homestead)", ES = "Ocultar Hojas (Homestead)" },["No fog"] = { PT = "Sem Neblina", ES = "Sin Niebla" },["Black Fog"] = { PT = "Neblina Escura", ES = "Niebla Oscura" },["Gray characters"] = { PT = "Personagens Cinzas", ES = "Personajes Grises" },["Floorbang"] = { PT = "Floorbang (Atravessar Chão)", ES = "Floorbang (Atravesar Suelo)" },
    
    -- Textures Page["Textures Settings"] = { PT = "Configurações de Textura", ES = "Ajustes de Texturas" },
    ...
    ["QUICK SELECT"] = { PT = "SELEÇÃO RÁPIDA", ES = "SELECCIÓN RÁPIDA" },["Skin Found!"] = { PT = "Skin Encontrada!", ES = "¡Skin Encontrada!" },
    ["Username..."] = { PT = "Nome de Usuário...", ES = "Nombre de Usuario..." },
    ...
    ["Join Random Server"] = { PT = "Entrar em Serv. Aleatório", ES = "Entrar en Serv. Aleatorio" },
    
    -- Configurações e Caixas de Diálogo (Modals)
    ...
    ["Reset"] = { PT = "Redefinir", ES = "Restablecer" },
    ["Set Key"] = { PT = "Definir Tecla", ES = "Fijar Tecla" },
}
...
function TranslationModule.Translate(ScreenGui, Lang)
    for _, obj in pairs(ScreenGui:GetDescendants()) do
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
            ...
            -- Salva o texto original no cache do objeto (atributo) se for a primeira vez
            if obj.Text and obj.Text ~= "" and not obj:GetAttribute("OrigText") then
                -- Ignora textos dinâmicos que mudam sempre
                if not string.find(obj.Text, "FPS:") and not string.find(obj.Text, "NexVoid V2.3") then
                    obj:SetAttribute("OrigText", obj.Text)
                end
            end
            
            if obj:IsA("TextBox") and obj.PlaceholderText and obj.PlaceholderText ~= "" and not obj:GetAttribute("OrigPlaceholder") then
            ...
            if orig then
                -- Se a tradução direta existir no dicionário
                if Dictionary[orig] then
                    obj.Text = Lang == "EN" and orig or Dictionary[orig][Lang]
                else
                    -- Lógica inteligente para Dropdowns ex: "Font Changer: Arial"

return function(env)
    ...
    Library:CreateToggle(Page, "Computer Progress", false, function(state)
        ...
        local function setupComputer(tableModel)
            if not tableModel or not tableModel.Parent then return end
    ...
    Library:CreateToggle(Page, "Door Progress", false, function(state)
        ...
        if state then
            ...
            local function setupNormalDoor(doorModel)
                if not doorModel or not doorModel.Parent then return end
            ...
            local function scanAndSetupDoors()
                cleanupNormalDoors()
                if not doorProgressActive then return end
            ...
            if CurrentMap then
                DoorProgLoop = CurrentMap.Changed:Connect(function()
                    task.wait(0.5)
                    scanAndSetupDoors()
                end)
            end
            ...
            if DoorProgLoop then 
                if typeof(DoorProgLoop) == "RBXScriptConnection" then
                    DoorProgLoop:Disconnect()
                else
                    task.cancel(DoorProgLoop)
                end
                DoorProgLoop = nil 
            end
    ...
    Library:CreateToggle(Page, "ExitDoor Progress", false, function(state)
        ...
        if state then
            ...
            local function scanForExitDoors()
                ...
                for _, obj in ipairs(searchArea:GetChildren()) do
    ...
    Library:CreateToggle(Page, "GetUp Timer", false, function(state)
        ...
        if state then
            ...
            local createBillboardCountdown = function(p, head)
                ...
                billboard.Enabled = not hideHeadGetUp

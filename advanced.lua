return function(env)
    ...
    local targetHitboxSize = Vector3_new(2, 2, 2)
    local hbLoop = nil
    ...
    local autoTieCrosshairDistancia = 15 -- Distância independente para o Crosshair
    ...
    local function arV2Unswim()
        ...
        if hum then
            ...
            for i = 1, #enums do 
                hum:SetStateEnabled(enums[i], true) 
            end
        ...
        arV2Swimbeat = RunService.Heartbeat:Connect(function()
            pcall(function()
                ...
                if root and hum then
                    local moving = (hum.MoveDirection ~= Vector3_new() or UserInputService:IsKeyDown(Enum.KeyCode.Space))
                    if not moving then
    ...
    local function arV2Monitor(char)
        ...
        arV2HpConn = hum.HealthChanged:Connect(function(health)
            if arV2Enabled and health < arV2LastHealth and health > 0 then
                if not arV2Swimming then task_spawn(arV2Swim) end
            end
    ...
    local function enforceOfficialSync()
        ...
        if dist > 1 then 
            ...
            cam.CFrame = rawCFrame * CFrame_new(1.75, 0, 0)
            cam.Focus = cam.CFrame * CFrame_new(0, 0, -dist)
        end
    end
    ...
    local function updateTargetHitboxSize()
        targetHitboxSize = Vector3_new(hbSizeX, hbSizeY, hbSizeZ)
    end
    ...
    Library:CreateToggle(Page, "Anti Ragdoll V2", false, function(state)
        arV2Enabled = state
        if state then
            if LocalPlayer.Character then task_spawn(arV2Monitor, LocalPlayer.Character) end
            ...
            task_spawn(function()
                ...
                while getgenv().TouchFlingEnabled do
                    ...
                    if RootPart then
                        ...
                        RootPart.Velocity = (oldVel * 9e8) + Vector3_new(0, 9e8, 0)
                        RunService.RenderStepped:Wait()
                        if RootPart then RootPart.Velocity = oldVel end
                        RunService.Stepped:Wait()
                        if RootPart then
                            RootPart.Velocity = oldVel + Vector3_new(0, movel, 0)
                            movel = movel * -1
                        end
                    end
                end
    ...
    Library:CreateToggle(Page, "No Hack Fail", false, function(state) 
        ...
        if state then
            if noHackFailThread then task_cancel(noHackFailThread) end
            noHackFailThread = task_spawn(function()
            ...
            if noHackFailThread then task_cancel(noHackFailThread) noHackFailThread = nil end
        end
    end)
                ...
                local function RemoverVentBlocks()
                    ...
                    for i = 1, #descendants do
                        ...
                        if obj:IsA("BasePart") and obj.Name == "VentBlock" then
                            obj.CFrame = CFrame_new(0, -10000, 0)
    ...
    local function EstaOlhandoPara(raizAlvo)
        ...
        local direcaoParaAlvo = (raizAlvo.Position - cameraCF.Position).Unit
        local direcaoOlhar = cameraCF.LookVector
        return direcaoOlhar:Dot(direcaoParaAlvo) >= lookSensitivity
    end
                                    ...
                                    if alvoCaido and alvoCapturado then
                                        if alvoCaido.Value == true and alvoCapturado.Value == false then
                                            ...
                                            if RaizAlvo then
                                                ...
                                                if distancia <= autoTieCrosshairDistancia and EstaOlhandoPara(RaizAlvo) then
    ...
    Library:CreateSlider(Page, "Auto Tie Crosshair Range", 5, 30, 15, function(val)
        autoTieCrosshairDistancia = val
    end)
    ...
    Library:CreateToggle(Page, "Runner Speed Boost", false, function(state)
        ...
        if state then
            ...
            pcall(function()
                local MeuPersonagem = LocalPlayer.Character
                local Humanoid = MeuPersonagem and MeuPersonagem:FindFirstChildWhichIsA("Humanoid")
                if Humanoid then
                    Humanoid.WalkSpeed = wsEnabled and wsValue or 16
                end
            end)
    ...
    Library:CreateToggle(Page, "Hitbox Extender", false, function(state) 
        ...
        if state then 
            ...
            if hbLoop then task_cancel(hbLoop) hbLoop = nil end
            local players = Players:GetPlayers()
            for i = 1, #players do 
                local v = players[i]
    ...
    Library:CreateToggleKeybind(Page, "Fly", false, "None", function(state) 
        ...
        local function setupFly(char)
            ...
            if flyEnabled then
                ...
                flyBv.Velocity = Vector3_new(0, 0, 0)
                ...
                flyConnection = RunService.RenderStepped:Connect(function()
                    if flyEnabled and flyBg and flyBv and LocalPlayer.Character then
                        ...
                        if charHrp and charHum then
                            ...
                            flyBg.CFrame = CFrame_new(charHrp.Position, charHrp.Position + Camera.CFrame.LookVector)
                            local moveDir = charHum.MoveDirection
                            if moveDir.Magnitude > 0 then
                               _G.flatLook = Vector3_new(Camera.CFrame.LookVector.X, 0, Camera.CFrame.LookVector.Z).Unit
                                local camLook = Camera.CFrame.LookVector
                                local camRight = Camera.CFrame.RightVector
                                local flatLook = Vector3_new(camLook.X, 0, camLook.Z).Unit
                                local flatRight = Vector3_new(camRight.X, 0, camRight.Z).Unit
    ...
    Library:CreateToggle(Page, "Fast Double Jump", false, function(state)
        ...
        if state then
            ...
            local function cloud(r)
                ...
                p.Massless=true
                p.Anchored=true
                p.CFrame=CFrame_new(r.Position.X,r.Position.Y-3.2,r.Position.Z)*r.CFrame.Rotation
                p.Parent=workspace
                ...
                pe.Acceleration=Vector3_new(0,4,0)
            ...
            local function spark(c)
                ...
                w.Part0=p
                w.Part1=a
                w.C0=g and CFrame_new() or CFrame_new(.4,.6,.7)
                w.Parent=p
            ...
            local function start(c)
                ...
                local hb=R.Heartbeat:Connect(function()
                    ...
                    if cd and time()-ld>=CD then
                        cd=false
                        task_spawn(function()spark(c)end)
                    end
                    j=h.Jump
                end)
                ...
                local jm=U.JumpRequest:Connect(function()
                    if not h or not r or not r.Parent then return end
                        ...
                        local ok,hit=pcall(function()
                            return workspace:Blockcast(r.CFrame,Vector3_new(2.2,2,1.4),Vector3_new(0,-16,0),p)
                        end)
                        ...
                        and time()-ld>=CD then
                            ...
                            air=false
                            r.AssemblyLinearVelocity=Vector3_new(0,JP,0)
                            ld=time()
                            cd=true
                            cloud(r)
                        end
            ...
            if plr.Character then task_spawn(start, plr.Character) end
            ...
            if not noclipConnection then
                noclipConnection = RunService.Stepped:Connect(function()
                    if LocalPlayer.Character then
                        local descendants = LocalPlayer.Character:GetDescendants()
                        for i = 1, #descendants do
                    ...
                    if infJumpEnabled then
                        pcall(function()
                            ...
                            if humanoid and hrp then
                                ...
                                hrp.AssemblyLinearVelocity = Vector3_new(currentVel.X, jumpPowerValue, currentVel.Z)
                            end
                        end)
                    end
                end)

return function(env)
    ...
    local function formatID(id)
        ...
        elseif type(id) == "string" and id ~= "" and id ~= "0" then
            if not string_find(id, "rbxassetid://", 1, true) then return "rbxassetid://" .. id else return id end
        end
        return nil
    end
    ...
    for i = 1, #initialPlayers do
        setupPlayerSoundEvents(initialPlayers[i])
    end
    ...
    local function isHackSound(sound) 
        ...
        for i = 1, #HACK_KEYWORDS do 
            if string_find(name, HACK_KEYWORDS[i], 1, true) then return true end 
        end
        return false 
    end
    ...
    local function muteHack(sound) 
        ...
        table_insert(hackSignals, {Signal = sig, Object = sound}) 
    end
    ...
    local function isHitSoundTarget(soundName)
        ...
        for i = 1, #HIT_KEYWORDS do
            if string_match(lowerName, HIT_KEYWORDS[i]) then return true end
        end
        return false
    end
    ...
    local function muteIfHitSound(obj)
        if obj:IsA("Sound") and isHitSoundTarget(obj.Name) then
            ...
            table_insert(noHitSoundSignals, {Signal = sig, Object = obj})
        end
    end
    ...
    local function getSoundCategory(sound)
        local name = stringLower(sound.Name)
        if string_find(name, "running", 1, true) or string_find(name, "walk", 1, true) or string_find(name, "step", 1, true) then
            return "Footsteps"
        elseif string_find(name, "jumping", 1, true) or string_find(name, "jump", 1, true) then
            return "Jump"
        elseif string_find(name, "landing", 1, true) or string_find(name, "fall", 1, true) or string_find(name, "land", 1, true) then
            return "Fall"
        end
        return nil
    end
    ...
    for i = 1, #workspaceDescendants do
        registerSound(workspaceDescendants[i])
    end
    Workspace.DescendantAdded:Connect(registerSound)

    local function setSoundVolumeDirect(sound, vol)
        sound.Volume = vol
    end
            ...
            for sound in pairs(ActiveSounds) do
                ...
                if category then
                    ...
                    if sound.Volume ~= targetVol then
                        pcall(setSoundVolumeDirect, sound, targetVol)
                    end
                end
            end
        end
    ...
    local function toggleMuteBeast(state)
        ...
        if state then
            ...
            for i = 1, #wsDesc do
                handleGlobalSoundAdded(wsDesc[i])
            end
            local ssDesc = SoundService:GetDescendants()
            for i = 1, #ssDesc do
                handleGlobalSoundAdded(ssDesc[i])
            end
            ...
            table_insert(MuteBeastConns, c1)
            table_insert(MuteBeastConns, c2)
            ...
            table_insert(MuteBeastConns, loopThread)
        else
            for i = 1, #MuteBeastConns do
                local c = MuteBeastConns[i]
    ...
    CreateCompactToggle(MuteBlock, "Remove Pc Hack Sounds", false, function(state) 
        if state then 
            ...
            for i = 1, #hackSignals do
                local data = hackSignals[i]
    ...
    CreateCompactToggle(MuteBlock, "No hit sound", false, function(state)
        ...
        if state then
            local function monitorCharacter(character)
                local charDesc = character:GetDescendants()
                for i = 1, #charDesc do
                    muteIfHitSound(charDesc[i])
                end
            ...
            local ssDesc = game:GetService("SoundService"):GetDescendants()
            for i = 1, #ssDesc do
                muteIfHitSound(ssDesc[i])
            end
            ...
            for i = 1, #noHitSoundSignals do
                local data = noHitSoundSignals[i]
                if data.Signal then data.Signal:Disconnect() end
                if data.Object then data.Object.Volume = 0.5 end
            end
            noHitSoundSignals = {}
    ...
    playBtn.MouseButton1Click:Connect(function()
        if isPlaying then
            ...
            local cleanId = string_match(text, "%d+")
            if cleanId then
                MusicSound:Stop()
    ...
    local function CreateSoundCard(Parent, TitleText, Actions)
        ...
        local btnWidth = (1 / totalActions)
        for i = 1, totalActions do
            local act = Actions[i]
            local btn = InstanceNew("TextButton")
            btn.Size = UDim2new(btnWidth, -((totalActions-1)*8 / totalActions), 1, 0)
